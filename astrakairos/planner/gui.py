...
"""
    ...
    def _validate_filter_range(self, filter_name: str, min_val, max_val, allow_empty: bool = False) -> tuple:
        """Legacy validation function for backward compatibility.
        
        Args:
            filter_name: Name of the filter
            min_val: Minimum value (can be None, empty string, or number)
            max_val: Maximum value (can be None, empty string, or number)
            allow_empty: Whether to allow empty (None) values
            
        Returns:
            tuple: (is_valid, min_val, max_val, search_method)
        """
        if filter_name not in STELLE_DOPPIE_FILTERS:
            return False, min_val, max_val, '1'
        
        config = STELLE_DOPPIE_FILTERS[filter_name]
        
        # Normalize empty values to None
        if min_val == '' or min_val == 0 or min_val is None:
            min_val = None
        if max_val == '' or max_val == 0 or max_val is None:
            max_val = None
            
        # Convert string values to float if needed
        try:
            if min_val is not None:
                # Handle cases where DoubleVar might return empty string
                if isinstance(min_val, str) and min_val.strip() == '':
                    min_val = None
                else:
                    min_val = float(min_val)
            if max_val is not None:
                # Handle cases where DoubleVar might return empty string
                if isinstance(max_val, str) and max_val.strip() == '':
                    max_val = None
                else:
                    max_val = float(max_val)
        except (ValueError, TypeError):
            messagebox.showerror("Invalid Filter Value", 
                               f"Invalid numeric value in {filter_name.title()} filter")
            return False, min_val, max_val, '1'
        
        # Handle empty values
        if allow_empty:
            if min_val is None and max_val is None:
                return False, min_val, max_val, '1'  # Both empty, no filter
            elif min_val is None:
                # Only max specified - "less than or equal" search
                if 'max_value' in config and not (config.get('min_value', float('-inf')) <= max_val <= config['max_value']):
                    messagebox.showerror("Invalid Filter Range", 
                                       f"{filter_name.title()} maximum value {max_val} outside valid range "
                                       f"[{config.get('min_value', 'unlimited')}, {config['max_value']}]")
                    return False, min_val, max_val, '1'
                return True, config.get('min_value', max_val), max_val, '6'  # Less or equal to
            elif max_val is None:
                # Only min specified - "more or equal to" search
                if 'min_value' in config and not (config['min_value'] <= min_val <= config.get('max_value', float('inf'))):
                    messagebox.showerror("Invalid Filter Range", 
                                       f"{filter_name.title()} minimum value {min_val} outside valid range "
                                       f"[{config['min_value']}, {config.get('max_value', 'unlimited')}]")
                    return False, min_val, max_val, '1'
                return True, min_val, config.get('max_value', min_val), '4'  # More or equal to
        
        # Both values specified - validate normally
        if 'min_value' in config and not (config['min_value'] <= min_val <= config.get('max_value', float('inf'))):
            messagebox.showerror("Invalid Filter Range", 
                               f"{filter_name.title()} minimum value {min_val} outside valid range "
                               f"[{config['min_value']}, {config.get('max_value', 'unlimited')}]")
        ...
        return True, min_val, max_val, '7'  # Range search
